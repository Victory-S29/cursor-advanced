import React, { useState } from 'react';
import { useDispatch, useSelector } from 'react-redux';
import { changeCommentAction, changeLikeAction, changeRepostAction } from '../store/actions/post.action';
import "./Post.css"

function Post({ id, profileImg, postImg, authorName, authorNickname, postContent, postDate, likes, comments, reposts }) {
    const post = {
        author: {
            authorName,
            profileImg,
            authorNickname,
        },
        postContent,
        postImg,
        postDate,
        likes,
        comments,
        reposts,
    };

    const posts = useSelector((state) => state.post.posts)
    const [isLiked, setIsLiked] = useState(posts[id].isLiked);
    const [isCommented, setIsCommented] = useState(posts[id].isCommented);
    const [isReposted, setIsReposted] = useState(posts[id].isReposted);

    const dispatch = useDispatch();

    const onClickLike = () => {
        setIsLiked(!isLiked)
        if (!isLiked) {
            const likeObject = {
                id,
                isLiked: true,
            }
            dispatch(changeLikeAction(likeObject));
        } else {
            const likeObject = {
                id,
                isLiked: false,
            }
            dispatch(changeLikeAction(likeObject));
        }


    }

    const onClickComment = () => {
        setIsCommented(!isCommented)
        if (!isCommented) {
            const commentObject = {
                id,
                isCommented: true,
            }
            dispatch(changeCommentAction(commentObject));
        } else {
            const commentObject = {
                id,
                isCommented: false,
            }
            dispatch(changeCommentAction(commentObject));
        }
    }

    const onClickRepost = () => {
        setIsReposted(!isReposted)
        if (!isReposted) {
            const repostObject = {
                id,
                isReposted: true,
            }
            dispatch(changeRepostAction(repostObject));
        } else {
            const repostObject = {
                id,
                isReposted: false,
            }
            dispatch(changeRepostAction(repostObject));
        }
    }

    return (
        <div className='post'>
            <div className='main-title'>
                <img className='bio-img' src={post.author.profileImg} alt='author'></img>
                <p className='bio-name'>{post.author.authorName}</p>
                <p className='bio-nickname'>{post.author.authorNickname}</p>
                <p className='post-date'>{post.postDate}</p>
            </div>
            <p className='post-content'>{post.postContent}</p>
            <img className='post-img' src={post.postImg} alt='post'></img>
            <div className='reaction-section'>
                <div className='reaction-section--part'>
                    <img onClick={onClickLike} className='reaction-img' src='data:image/png;base64,' alt='like'></img>
                    <p className='reaction-p'>{likes}</p>
                </div>
                <div className='reaction-section--part'  >
                    <img onClick={onClickRepost} className='reaction-img' src='data:image/png;base64,' alt='retweet'></img>
                    <p className='reaction-p'>{reposts}</p>
                </div>
                <div className='reaction-section--part'>
                    <img onClick={onClickComment} className='reaction-img' src='https://cdn2.iconfinder.com/data/icons/medical-healthcare-26/28/Chat-2-512.png' alt='comments'></img>
                    <p className='reaction-p'>{comments}</p>
                </div>
                <div className='reaction-section--part'>
                    <img className='reaction-img' src='data:image/png;base64,' alt='download'></img>
                </div>
            </div>
        </div>
    )
}

export default Post;